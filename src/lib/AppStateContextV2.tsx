    ...
    const refreshData = async () => {
        ...
        try {
            ...
            for (const ca of childAccessData as any[]) {
                if (ca.children_v2) {
                    const avatarUrl = await getAvatarUrl(ca.children_v2.avatar_url);
                    loadedChildren.push({
                        id: ca.children_v2.id,
                        name: ca.children_v2.name,
                        avatarInitials: ca.children_v2.name?.[0]?.toUpperCase() || "?",
                        avatarUrl,
                        dob: ca.children_v2.dob,
                    });
                }
            }
            ...
            if (childSpacesData) {
                for (const cs of childSpacesData as any[]) {
                    loadedChildSpaces.push({
                        ...
                        childId: cs.child_id,
                        homeId: cs.home_id,
                        homeName: cs.homes_v2?.name || "Unknown",
                    });

                    if (cs.homes_v2) {
                        homeIds.push(cs.homes_v2.id);
                        const photoUrl = await getAvatarUrl(cs.homes_v2.photo_url);
                        loadedHomes.push({
                            id: cs.homes_v2.id,
                            name: cs.homes_v2.name,
                            address: cs.homes_v2.address,
                            photoUrl,
                            notes: cs.homes_v2.notes,
                            childSpaceId: cs.id,
            ...
            // Also load pending invites from invites_v2
