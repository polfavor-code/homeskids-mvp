...
interface ItemsContextType {
    ...
    addItem: (item: AddItemInput) => Promise<{ success: boolean; error?: string; item?: ItemV2 }>;

    updateItem: (itemId: string, updates: Partial<ItemV2>) => Promise<void>;
    deleteItem: (itemId: string) => Promise<{ success: boolean; error?: string }>;

    // Location/status updates
    updateItemStatus: (itemId: string, status: ItemV2["status"]) => Promise<void>;
    ...
    // Filter items
    getItemsByChildSpace: (childSpaceId: string) => ItemV2[];
    getItemsByStatus: (status: ItemV2["status"]) => ItemV2[];
    getItemsOriginatedByUser: (userId: string) => ItemV2[];
...
export function ItemsProvider({ children }: { children: ReactNode }) {
    ...
    const fetchItems = useCallback(async () => {
        try {
            ...
            if (itemsData) {
                ...
                const mappedItems: ItemV2[] = itemsData.map((item: any) => ({
                    id: item.id,
    ...
    // Realtime subscription for items_v2
    useEffect(() => {
    ...
    const addItem = async (item: AddItemInput): Promise<{ success: boolean; error?: string; item?: ItemV2 }> => {
        try {
    ...
    // Update item
    const updateItem = async (itemId: string, updates: Partial<ItemV2>) => {
    ...
    // Update status
    const updateItemStatus = async (itemId: string, status: ItemV2["status"]) => {
    ...
    const getItemsByStatus = (status: ItemV2["status"]) =>
